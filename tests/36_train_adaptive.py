...
# --- CONFIGURAZIONE ADAPTIVE (V4) 🧠 ---
# Usiamo il dataset V4 che contiene sia movimenti "Macro" (Lookahead 10) che "Micro" (Lookahead 1)
DATASET_PATH = "dataset_finetuning_v4/dataset.json" 
OUTPUT_DIR = "checkpoints_franka_adaptive"

# Parametri Iper-Ottimizzati (Identici al Diamond per confrontare le performance)
BATCH_SIZE = 4          
GRAD_ACCUMULATION = 4   
LEARNING_RATE = 5e-5    
MAX_LENGTH = 512
EPOCHS = 1              
SAVE_STEPS = 500        
DEBUG_EVERY = 50        # Log verbose ogni 50 step

# Range Azione (Sempre -1 a 1 grazie allo scaling x5 che abbiamo mantenuto nel generatore V4)
ACTION_MIN = -1.0 
ACTION_MAX = 1.0
...
class AdaptiveDataset(Dataset):
    def __init__(self, json_path, processor):
        print(f">>> Caricamento Dataset ADAPTIVE (V4): {json_path}")
        with open(json_path, 'r') as f:
            self.data = json.load(f)
        
        # Path root per le immagini (che nel JSON V4 puntano a ../dataset_finetuning_v2/...)
        ...
        # Statistica rapida per confermare che abbiamo caricato il V4
        sample_act = np.array(self.data[0]['action'])
        print(f"Esempio Azione Raw: {sample_act[:3]} ...")
    ...
    def __getitem__(self, idx):
        ...
        # Gestione path immagini relativi
        img_path = os.path.join(self.root_dir, item['image_path'])
        
        try:
            image = Image.open(img_path).convert("RGB")
        except Exception:
            # Fallback nero (solo per sicurezza estrema)
...
def safe_debug_print(processor, logits, labels, step):
    ...
    try:
        ...
        # Cerchiamo di capire se è un movimento GRANDE o PICCOLO
        if len(t_nums) >= 3:
            movement_mag = np.mean(np.abs(np.array(t_nums[:3]) - 127))
            move_type = "VELOCE 🚀" if movement_mag > 10 else "FINO 🤏"
            print(f"📊 Tipo Movimento Richiesto: {move_type} (Mag: {movement_mag:.1f})")
            
        if len(t_nums) >= 3 and len(p_nums) >= 3:
            diff = np.abs(np.array(t_nums[:3]) - np.array(p_nums[:3]))
            print(f"📉 Delta Errore: {diff}")
    except:
        pass

    # Check "Sindrome del 127"
    if "127 127 127" in pred_str:
        print("⚠️  Warning: Predizione STATICA (127).")
    else:
        print("✅  OK: Il modello predice MOVIMENTO.")
    
    print("-" * 50)

def train():
    print(f">>> AVVIO TRAINING ADAPTIVE 🧠 (V4 Dataset)")
    ...
    # Caricamento Dataset V4
    dataset = AdaptiveDataset(DATASET_PATH, processor)
    ...
    for epoch in range(EPOCHS):
        ...
        for step, batch in enumerate(progress):
            ...
            if (step + 1) % GRAD_ACCUMULATION == 0:
                ...
                # Update Barra
                avg_loss = total_loss # Loss già divisa per accumulo
