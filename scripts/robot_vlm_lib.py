...
class PandaSafeDriver:
    ...
    def get_real_tip_pose(self) -> np.ndarray:
        """
        Cinematica diretta polso + offset sul tip (approssimazione utile).
        Restituisce coordinate world-frame.
        """
        ...
        tip = wrist_pos + wrist_mat.dot(np.array([0, 0, 0.13]))
...
class VLMRobotInterface:
    ...
    def get_context(self) -> Tuple[Dict[str, Any], Dict[str, np.ndarray]]:
        ...
        text_info = {
            "eef_xyz": [round(float(x), 3) for x in tip_pos],
    ...
    def execute_action(self, primitive: str, params: list) -> Dict[str, Any]:
        """
        Esegue primitiva. Ritorna info sullo stato finale + METRICA DI PROGRESSO (EEF delta).
        NON cambia le primitive: aggiunge solo misure per capire se l'azione ha avuto effetto.
        """
        base_cmd = [0, 0, 0]
        arm_cmd = [0, 0, 0, 0, 0, 0]
        torso_cmd = 0.0
        ...
        # misura progresso (prima)
        start_tip = self.driver.get_real_tip_pose()

        try:
            if primitive == 'base':
                base_cmd = [float(p) * self.BASE_SENSITIVITY for p in params]
            elif primitive == 'arm':
                arm_cmd[0] = float(params[0]) * self.ARM_SENSITIVITY
                arm_cmd[1] = float(params[1]) * self.ARM_SENSITIVITY
                arm_cmd[2] = float(params[2]) * self.ARM_SENSITIVITY
                ...
                if isinstance(val, str):
                    ...
                    self.gripper_state = 1.0 if float(val) > 0 else -1.0
            ...
            for _ in range(self.STEPS_PER_ACTION):
                ...
                if is_crit and primitive == 'arm':
                    clamped_count += 1

                self.last_obs, _, _, _ = self.env.step(action)
                self.env.render()

            # misura progresso (dopo)
            end_tip = self.driver.get_real_tip_pose()
            delta = (end_tip - start_tip).astype(np.float32)
            ...
            # Aggiunte: metriche oggettive di spostamento
            final_info["eef_delta_xyz"] = [round(float(x), 4) for x in delta]
            final_info["eef_delta_norm"] = round(delta_norm, 5)
